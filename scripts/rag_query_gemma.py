...
or import it in FastAPI (app.py):
    from rag_query_gemma import ask_gemma
"""
...
GEMMA_MODEL = "google/medgemma-4b-it"  # text-generation model
# ----------------


# ---- EMBEDDING MODEL ----
def get_embedding_model():
    """Load the CLIP model and processor for text embeddings."""
...
def get_gemma_pipeline():
    """Load the Gemma language model pipeline."""
...
def retrieve_context(qdrant_client, query_vec):
    """Retrieve top-k most relevant text chunks from Qdrant."""
...
def ask_gemma(query: str) -> str:
    """
    Handle a chatbot question via backend API.
    This wraps the same RAG logic for FastAPI or other apps.
    """
